...
import app.yokaicore.util.lang.truncateCenter
...
fun Context.openInBrowser(uri: Uri, forceDefaultBrowser: Boolean = false) {
    try {
        val intent = Intent(Intent.ACTION_VIEW, uri).apply {
            // Force default browser so that verified extensions don't re-open YokaiCore
...
fun Context.createReaderThemeContext(): Context {
    ...
    if (resources.configuration.uiMode and Configuration.UI_MODE_NIGHT_MASK != expected) {
        ...
        val wrappedContext = ContextThemeWrapper(this, R.style.Theme_YokaiCore)
