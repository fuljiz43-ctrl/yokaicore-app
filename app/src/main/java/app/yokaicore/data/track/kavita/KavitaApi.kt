package app.yokaicore.data.track.kavita
...
class KavitaApi(private val client: OkHttpClient, interceptor: KavitaInterceptor) {
    ...
    fun getNewToken(apiUrl: String, apiKey: String): String? {
        val request = POST(
            "$apiUrl/Plugin/authenticate?apiKey=$apiKey&pluginName=YokaiCore-Kavita",
    ...
    private fun getLatestChapterRead(url: String): Float {
        ...
        val requestUrl = "${getApiFromUrl(url)}/YokaiCore/latest-chapter?seriesId=$seriesId"
        try {
            with(json) {
    ...
    suspend fun updateProgress(track: Track): Track {
        val requestUrl = "${getApiFromUrl(
            track.tracking_url,
        )}/YokaiCore/mark-chapter-until-as-read?seriesId=${getIdFromUrl(
            track.tracking_url,
