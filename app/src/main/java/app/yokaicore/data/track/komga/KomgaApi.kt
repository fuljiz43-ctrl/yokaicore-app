...
class KomgaApi(
    ...
    suspend fun getTrackSearch(url: String): TrackSearch =
        withIOContext {
            try {
                ...
                val progress = client
                    .newCall(GET("${url.replace("/api/v1/series/", "/api/v2/series/")}/read-progress/yokaicore"))
                    .awaitSuccess().let {
                        with(json) {
    ...
    suspend fun updateProgress(track: Track): Track {
        ...
        client.newCall(
            Request.Builder()
                .url("${track.tracking_url.replace("/api/v1/series/", "/api/v2/series/")}/read-progress/yokaicore")
